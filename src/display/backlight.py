...
class DisplayBacklight:
    ...
    def __init__(self):
        ...
        # Get brightness levels from config or use defaults
        self.brightness_levels = config.get('brightness_levels', [
    ...
    def _brightness_value(self, value: float) -> int:
        """
    ...
    def set_brightness(self, raw_value: float) -> None:
        ...
        try:
            lcd_pwm_value = self._brightness_value(raw_value)
            self.pi.set_PWM_dutycycle(self.pin, lcd_pwm_value)
            logger.debug(f"Set brightness raw:{raw_value:.3f} lcd_pwm_value:{lcd_pwm_value}")
        except Exception as e:
    ...
    def step_brightness(self) -> None:
        """Steps brightness down by one level per call; cycles back to 100% after 0%"""
        if not self.pi or not self.pi.connected:
        ...
        try:
            ...
            # Set the new brightness from the levels list
